...
test('paired07: R membership requires explicit new policy and dispatches exactly the frozen new-model body',async()=>{
  const {FLASH41_PAIRED07_POLICY}=await import('./real-input-budget.mjs')
  const unitId='R07-07',value=JSON.parse(request());value.model='deepseek-flash'
  const body=JSON.stringify(value),identity=inspectRequest(body,FLASH41_PAIRED07_POLICY)
  let secretReads=0,calls=0,reserves=0,records=0
  const envelope=JSON.parse(raw(unitId));envelope.model='deepseek-flash'
  const config={origin:ORIGIN,capability:CAP,requests:{[unitId]:body},clock:()=>NOW,
    budget:{snapshot:async()=>({units:[{unitId,...identity}]}),reserve:async(id,text,policy)=>{
      assert.equal(id,unitId);assert.equal(text,body);assert.deepEqual(policy,FLASH41_PAIRED07_POLICY);reserves++
      return{complete:async()=>({usage:envelope.usage,costUpperMicroCny:280}),uncertain:async()=>{throw Error('UNEXPECTED')}}}},
    readSecret:()=>{secretReads++;return FAKE_SECRET},fetchImpl:async(url,options)=>{
      calls++;assert.equal(url,'https://api.deepseek.com/responses');assert.equal(options.body,body);return http(JSON.stringify(envelope))},
    recordRaw:async row=>{records++;assert.equal(row.unitId,unitId);assert.equal(row.requestSha,identity.requestSha)}}
  for(const policy of [BILLING_POLICY,FLASH41_POLICY])await assert.rejects(()=>createModelGateway({...config,policy}),/PAIRED07_POLICY/)
  assert.equal(secretReads,0);assert.equal(calls,0)
  const gateway=await createModelGateway({...config,policy:FLASH41_PAIRED07_POLICY})
  const message={method:'POST',path:'/api/real-input/recognize',headers:{host:new URL(ORIGIN).host,origin:ORIGIN,
    'sec-fetch-site':'same-origin','content-type':'application/json','x-real-input-capability':CAP},bodyText:JSON.stringify({unitId,requestSha:identity.requestSha})}
  for(const other of ['R07-03','R13-07','R07-05','P07-05']){
    assert.equal((await gateway.handle({...message,bodyText:JSON.stringify({unitId:other,requestSha:identity.requestSha})})).status,400)
  }
  assert.equal(secretReads,0)
  assert.equal((await gateway.handle(message)).status,200)
...
test('paired08: S/T membership requires the new policy and keeps text-only identity',async()=>{
  const {FLASH41_PAIRED07_POLICY,FLASH41_PAIRED08_POLICY}=await import('./real-input-budget.mjs'),unitId='S01-08'
  const value=JSON.parse(request());value.model='deepseek-flash';const body=JSON.stringify(value),identity=inspectRequest(body,FLASH41_PAIRED08_POLICY)
  let calls=0,reserves=0,records=0;const envelope=JSON.parse(raw(unitId));envelope.model='deepseek-flash'
  const config={origin:ORIGIN,capability:CAP,requests:{[unitId]:body},clock:()=>NOW,
    budget:{snapshot:async()=>({units:[{unitId,...identity}]}),reserve:async(id,text,policy)=>{assert.equal(id,unitId);assert.equal(text,body);assert.deepEqual(policy,FLASH41_PAIRED08_POLICY);reserves++;return{complete:async()=>({usage:envelope.usage,costUpperMicroCny:300}),uncertain:async()=>{throw Error('UNEXPECTED')}}}},
