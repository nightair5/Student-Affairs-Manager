...
async function main() {
  ...
  if (writeArtifacts) {
    ...
    await writeR10ImmutableArtifacts([
      { path: path.join(docsDir, 'protocol-bundle-h.json'), contents: `${JSON.stringify(protocolBundle, null, 2)}\n` },
      { path: path.join(docsDir, 'qualification-result-h.json'), contents: `${JSON.stringify(qualificationResult, null, 2)}\n` },
      { path: path.join(docsDir, 'qualification-evidence-h.json'), contents: `${JSON.stringify(evidence, null, 2)}\n` },
    ])
  }
