...
async function main() {
  ...
  const evidence = {
      schemaVersion: 'e2.9-r10-zero-model-evidence-1.2.2',
    protocolVersion: R10_PROTOCOL_VERSION,
  ...
  if (writeArtifacts) {
    ...
    await writeR10ImmutableArtifacts([
      { path: path.join(docsDir, 'protocol-bundle-d.json'), contents: `${JSON.stringify(protocolBundle, null, 2)}\n` },
      { path: path.join(docsDir, 'qualification-result-d.json'), contents: `${JSON.stringify(qualificationResult, null, 2)}\n` },
      { path: path.join(docsDir, 'qualification-evidence-d.json'), contents: `${JSON.stringify(evidence, null, 2)}\n` },
    ])
  }
