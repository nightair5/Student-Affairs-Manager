...
import { canonicalJson, hashBundle, sha256 } from './e2-9-r4-hash.mjs'
...
const RUN_ID = 'e29r4-run-20260813-a'
const RUN_LABEL = 'e29r4-20260813-a'
const SEED = 'e2-9-r4-interleave-20260813-a'
  ...
  return [...specs].sort((a, b) => sha256(`${SEED}:${phase}:${a.caseId}`).localeCompare(sha256(`${SEED}:${phase}:${b.caseId}`))).flatMap((spec) => {
    ...
    return (proFirst ? ['pro', 'flash'] : ['flash', 'pro']).map((modelAlias) => {
      ...
      return {
        ...
        observationId: `e29r4-${sha256(`${RUN_ID}:${caseAlias}:${modelAlias}`).slice(0, 32)}`,
...
async function main() {
  ...
  for (const name of outputs) {
    ...
    if (!(await missing(file))) throw new Error(`Refusing to overwrite frozen R4 artifact: ${path.relative(ROOT, file)}`)
  }
  ...
  try {
    ...
    const sourceOnly = {
      schemaVersion: 'e2.9-r4-source-only-3.2.0', protocolVersion: PROTOCOL_VERSION,
      generationBoundary: 'No expected answers, scores, labels, prior outputs, R3 observations, R3 packets, or R3 mappings are present.',
    ...
    const readinessManifest = {
      schemaVersion: 'e2.9-r4-readiness-manifest-3.2.0', protocolVersion: PROTOCOL_VERSION,
    ...
    const phaseManifest = (phase, specs) => ({
      schemaVersion: `e2.9-r4-${phase}-manifest-3.2.0`, protocolVersion: PROTOCOL_VERSION, frozenBeforeCalls: true,
    ...
    const bundles = {
      ...
      protocolAndDeployment: await hashBundle(ROOT, [
        ...
        'scripts/reveal-e2-9-r4-adjudication.mjs', 'scripts/evaluate-e2-9-r4-gate.mjs',
        'docs/e2-v4-pro-benchmark-r4/EXPERIMENT_PLAN.md', 'docs/e2-v4-pro-benchmark-r4/adjudication-rubric.json',
        'docs/e2-v4-pro-benchmark-r4/adjudication-packet.schema.json',
      ]),
    }
    const bundleManifest = {
      schemaVersion: 'e2.9-r4-bundle-manifest-3.2.0', protocolVersion: PROTOCOL_VERSION,
      ...
      framingRule: 'UTF-8 byte length framed path and content joined by the frozen R4 separator.',
      algorithm: 'SHA-256',
      coverage: 'Worker route/security, frozen provider adapter, R4 transport policy, benchmark wrapper, immutable ledger, Preview flags/binding, deployment configs, runner, scorer, tests, package lock and secret scanner.',
      bundles,
    }
    ...
    const readinessObservations = MODELS.flatMap((modelAlias) => [1, 2, 3].map((probeIndex) => {
      ...
      return {
        observationId: `e29r4-${sha256(`${RUN_ID}:readiness:${modelAlias}:${probeIndex}`).slice(0, 32)}`,
    ...
    const [pathMaskSource, packetSchemaSource, adjudicationRubricSource] = await Promise.all([
      readFile(path.join(ROOT, 'scripts', 'e2-9-r4-path-mask.mjs'), 'utf8'),
    ...
    const runManifestCore = {
      schemaVersion: 'e2.9-r4-run-manifest-3.2.0', protocolVersion: PROTOCOL_VERSION, runId: RUN_ID, runLabel: RUN_LABEL,
      createdAt: new Date().toISOString(), implementationCommit,
      labels: { readiness: 'e29r4-readiness-20260813-a', smoke: 'e29r4-smoke-20260813-a', screening: 'e29r4-screening-20260813-a', scoring: 'e29r4-scoring-20260813-a', adjudication: 'e29r4-adjudication-20260813-a' },
      r3EvidencePolicy: 'HISTORICAL_PROTOCOL_FAILURE_EVIDENCE_ONLY_NEVER_REUSED_OR_REINTERPRETED',
      frozen: {
        ...
        schemaVersion: '2.0', pipelineVersion: 'recognition-pipeline-2.2.1', validatorVersion: 'recognition-quality-2.1.0', normalizerVersion: 'e2-9-r4-role-aware-normalizer-3.2.0',
        temperature: 0, thinking: 'disabled', maxTokens: 6000, responseFormat: 'json_object', stream: false, router: 'BYPASSED', repair: 'DISABLED',
        pathMaskVersion: 'e2-9-r4-path-mask-3.2.0', packetSchemaVersion: 'e2.9-r4-adjudication-packet-3.2.0',
        adjudicationVersion: 'e2.9-r4-user-impact-rubric-1.0.0', commitmentVersion: 'e2-9-r4-assignment-commitment-1.0.0',
        chronologyVersion: 'e2-9-r4-adjudication-chronology-1.0.0', pathMaskSourceSha256: sha256(pathMaskSource),
        packetSchemaSha256: sha256(packetSchemaSource), adjudicationRubricSha256: sha256(adjudicationRubricSource),
      },
      retryPolicy: {
        version: 'e2-9-r4-transport-policy-3.2.0', eligibleError: 'UPSTREAM_JSON_TRUNCATED', maxProtocolRetries: 1, maxAttemptsPerFormalObservation: 2,
