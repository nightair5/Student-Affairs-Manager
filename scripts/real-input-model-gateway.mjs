...
import { BILLING_POLICY, FLASH41_POLICY, FLASH41_PAIRED06_POLICY, FLASH41_PAIRED07_POLICY, FLASH41_PAIRED08_POLICY, FLASH41_PAIRED09_POLICY,
  MODEL_COMPARE_POLICY, modelComparePolicyFor, sha256 } from './real-input-budget.mjs'
...
export async function createModelGateway({origin,capability,budget,requests,recordRaw,
  ...
  check(isDeepStrictEqual(policy,BILLING_POLICY)||isDeepStrictEqual(policy,FLASH41_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED06_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'POLICY')
  ...
  const unitPolicy=id=>{
    if(id.startsWith('W')||id.startsWith('X')){check(isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'MODEL_COMPARE_POLICY');return modelComparePolicyFor(id)}
    if(id.startsWith('U')||id.startsWith('V')){check(isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'PAIRED09_POLICY');return FLASH41_PAIRED09_POLICY}
    if(id.startsWith('S')||id.startsWith('T')){check(isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'PAIRED08_POLICY');return FLASH41_PAIRED08_POLICY}
    if(id.startsWith('R')){check(isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'PAIRED07_POLICY');return FLASH41_PAIRED07_POLICY}
    if(id.startsWith('Q')){check(isDeepStrictEqual(policy,FLASH41_PAIRED06_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY),'PAIRED06_POLICY');return FLASH41_PAIRED06_POLICY}
    return id.startsWith('P')?(isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)?FLASH41_POLICY:policy):BILLING_POLICY
  }
  ...
  return Object.freeze({
    async handle({method,path,headers,bodyText}) {
      ...
      try {
        ...
        check(typeof unitId==='string'&&/^(?:[ABCD]0[1-8]|N(?:0[1-9]|1[0-2])-(?:03|04)|P(?:0[1-9]|1[0-2])-(?:03|05)|Q(?:0[1-9]|1[0-2])-(?:03|06)|R(?:0[1-9]|1[0-2])-(?:03|07)|S(?:0[1-9]|1[0-2])-(?:03|08)|T0[1-8]-(?:03|08)|U(?:0[1-9]|1[0-2])-(?:03|09)|V0[1-8]-(?:03|09)|W(?:0[1-9]|1[0-2])-[AB]|X0[1-8]-[AB])$/.test(unitId)&&request&&request.requestSha===input.requestSha,'CLIENT_BINDING')
