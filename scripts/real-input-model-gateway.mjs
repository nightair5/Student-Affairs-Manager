...
export async function createModelGateway({origin,capability,budget,requests,recordRaw,
  ...
  const unitPolicy=id=>{
    ...
    if(id.startsWith('W')||id.startsWith('X')){check(isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current,'MODEL_COMPARE_POLICY');return modelComparePolicyFor(id)}
    if(id.startsWith('U')||id.startsWith('V')){check(isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current,'PAIRED09_POLICY');return FLASH41_PAIRED09_POLICY}
    if(id.startsWith('S')||id.startsWith('T')){check(isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current,'PAIRED08_POLICY');return FLASH41_PAIRED08_POLICY}
    if(id.startsWith('R')){check(isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current,'PAIRED07_POLICY');return FLASH41_PAIRED07_POLICY}
    if(id.startsWith('Q')){check(isDeepStrictEqual(policy,FLASH41_PAIRED06_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED09_POLICY)||isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current,'PAIRED06_POLICY');return FLASH41_PAIRED06_POLICY}
    return id.startsWith('P')?(isDeepStrictEqual(policy,MODEL_COMPARE_POLICY)||isDeepStrictEqual(policy,REASONING_COMPARE_POLICY)||current?FLASH41_POLICY:policy):BILLING_POLICY
  }
    ...
    async handle({method,path,headers,bodyText}) {
      ...
      try {
        ...
        const exchange=(async()=>{
          ...
          const responseLimit=isDeepStrictEqual(policy,REASONING_MAX_POLICY)||isDeepStrictEqual(policy,REASONING_LOW16_POLICY)?2097152:524288
