...
const root = 'docs/recognition-optimization/mainline-03-i1/'
...
function protect() {
  ...
  const additions = baseline.allowedNew
...
const reproduction = process.argv[2] === '--reproduction'
const targeted = process.argv[2] === '--targeted' || reproduction
if (!targeted && (process.argv[2] !== '--full' || !process.argv[3])) throw new Error('EXPLICIT_FULL_AND_NPM_CLI_REQUIRED')
protect()
const directory = mkdtempSync(join(tmpdir(), 'rco-mainline-03-i1-'))
...
const tests = ['node_modules/vitest/vitest.mjs', 'run',
  ...(reproduction ? ['src/experiments/mainline03/recognitionHandoff.test.ts', '-t', 'reproduces'] : targeted ? ['src/experiments/mainline03', 'src/experiments/mainline02', 'src/domain/v2/confirmationV2.test.ts', 'src/experiments/mainline01p1/confirmationHarness.test.ts', 'src/experiments/mainline01'] : []),
...
let okay = await run('isolated-browser-bundle-and-dependency-check', ['scripts/serve-mainline-03-i1.mjs', '--check'])
for (const [name, args] of checks) { if (!okay) break; if (!await run(name, args)) { okay = false; break } }
if (okay && !targeted) {
  try {
    ...
    for (const entry of outputs) {
      ...
      if (content.includes('rco-mainline-01-') || content.includes('UNREPRESENTABLE_CONDITION_STATE') || content.includes('mainline03-receipt-1')) findings.push(entry.name + ':experimental-entry-in-stable-bundle')
    }
...
if (okay && !targeted) okay = await run('dependency-audit', [process.argv[3], 'audit', '--audit-level=high', '--cache', join(directory, 'npm-audit-cache')])
