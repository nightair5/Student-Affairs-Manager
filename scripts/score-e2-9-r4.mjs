...
import { assertFourWayModelLineage, assertR4StagePrerequisite, assertRunManifestBinding, scorableFinalPayload, summarizeProtocolRetries } from './e2-9-r4-integrity.mjs'
...
async function main() {
  const [runRaw, sourceRaw, phaseRaw, bundleRaw, activationRaw, checkpointRaw, ledgerRaw, packetManifestRaw, labelsRaw, revealRaw] = await Promise.all([
  ...
  if (ledger.runManifestSha256 !== run.runManifestSha256 || ledger.runStatus !== 'COMPLETE' || ledger.stage !== 'PATH_MASK_PREVIEW_OPEN') throw new Error('LEDGER_NOT_SCORABLE')
  ...
  if (packetManifest.runManifestSha256 !== run.runManifestSha256 || labels.packetSha256 !== packetManifest.packetSha256
    || reveal.packetSha256 !== packetManifest.packetSha256 || reveal.labelsSha256 !== labels.labelsSha256) throw new Error('ADJUDICATION_BINDING_MISMATCH')
  assertR4StagePrerequisite('scoring', { labelsFrozen: Boolean(labels.labelsCompletedAt), chronologyValid: reveal.chronologyValid === true, commitmentVerified: reveal.commitmentVerified === true })
  ...
  try {
    ...
    const result = {
      schemaVersion: 'e2.9-r4-anonymous-screening-aggregate-3.2.0', protocolVersion: PROTOCOL_VERSION, phase: 'screening',
