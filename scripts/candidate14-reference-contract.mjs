...
export const CANDIDATE14_REFERENCE_VERSION='candidate14-reference-contract-5.1.0'
export const CANDIDATE14_SCORER_VERSION='candidate14-scoring-5.1.0'
...
export function validateCandidate14Reference(reference){
  ...
  for(const relation of reference.relations){check(['supersedes','cancels','amends'].includes(relation.type)&&ids.has(relation.targetTaskId)&&(relation.fromTaskId===null||ids.has(relation.fromTaskId)),'RELATION');check(typeof relation.continuitySlot==='string'&&relation.continuitySlot,'RELATION_SLOT')}
