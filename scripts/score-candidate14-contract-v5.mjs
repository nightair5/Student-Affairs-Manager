...
function scoreNoTaskFacts(reference,result){
  ...
  const checks=reference.noTaskFacts.map(fact=>{let pass=fact.scopeIds.every(id=>coverage.has(id));if(fact.kind==='event')pass=pass&&actualEvents.some(row=>normalize(row.title)===normalize(fact.value));if(fact.kind==='time')pass=pass&&actualTimes.some(row=>normalize(row.rawText)===normalize(fact.value));if(fact.kind==='location')pass=pass&&actualEvents.some(row=>normalize(row.location)===normalize(fact.value));if(['prohibition','quoted_third_party_directive','pending_announcement','information'].includes(fact.kind))pass=pass&&fact.scopeIds.every(id=>(result.informationScopeIds??[]).includes(id)||(result.unresolvedScopeIds??[]).includes(id));return {...fact,pass}})
  const exactCounts=actualEvents.length===eventFacts.length&&actualTimes.length===timeFacts.length
  return {pass:checks.every(row=>row.pass)&&exactCounts,checks,exactCounts}
}
