...
function assignments(expected,actual,index=0,used=new Set(),rows=[],out=[]){
  ...
  for(let i=0;i<actual.length;i++)if(!used.has(i)){used.add(i);rows.push(i);assignments(expected,actual,index+1,used,rows,out);rows.pop();used.delete(i)}
  return out
}
...
export function scoreCandidate14(referenceInput,result,{schemaValid=true,referenceValid=true}={}){
  ...
  const actual=result.tasks??[], candidates=assignments(reference.tasks,actual), evaluated=candidates.map(map=>{
    const checks=map.map((idx,i)=>idx===null?null:taskCheck(reference.tasks[i],actual[idx]))
  ...
  return {version:CANDIDATE14_SCORER_VERSION,status:'SCORED',complete:fn===0&&fp===0&&major===0&&forbidden===0&&noTaskPass,severity:{severe:0,major,forbidden},taskMetrics:{tp:best.passing,fp,fn},noTaskPass,matches}
}
