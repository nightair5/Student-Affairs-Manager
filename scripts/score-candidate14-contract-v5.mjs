...
function scoreNoTaskFacts(reference,result){
  ...
  const coverage=new Set([...(result.informationScopeIds??[]),...(result.unresolvedScopeIds??[]),...actualEvents.flatMap(row=>row.scopeIds??[]),...actualTimes.flatMap(row=>row.scopeIds??[])])
  ...
  const checks=reference.noTaskFacts.map(fact=>{let pass=fact.scopeIds.every(id=>coverage.has(id));if(fact.kind==='event')pass=pass&&actualEvents.some(row=>values(fact).includes(normalize(row.title))&&normalize(row.description??'')===''&&containsScopes(row,fact));if(fact.kind==='time')pass=pass&&actualTimes.some(row=>values(fact).includes(normalize(row.rawText))&&containsScopes(row,fact));if(fact.kind==='location')pass=pass&&actualEvents.some(row=>values(fact).includes(normalize(row.location))&&containsScopes(row,fact));if(['prohibition','quoted_third_party_directive','pending_announcement','information'].includes(fact.kind))pass=pass&&fact.scopeIds.every(id=>(result.informationScopeIds??[]).includes(id)||(result.unresolvedScopeIds??[]).includes(id));return {...fact,pass}})
  ...
  const actualLocations=actualEvents.filter(row=>normalize(row.location)).map(row=>normalize(row.location)).sort(),expectedLocations=locationFacts.map(row=>normalize(row.value)).sort()
  const exactCounts=actualEvents.length===eventFacts.length&&actualTimes.length===timeFacts.length,exactLocations=canonical(actualLocations)===canonical(expectedLocations),exactCoverage=canonical([...coverage].sort())===canonical([...expectedCoverage].sort()),exactTopLevel=canonical([...actualTopLevel].sort())===canonical([...expectedTopLevel].sort())
  return {pass:checks.every(row=>row.pass)&&exactCounts&&exactLocations&&exactCoverage&&exactTopLevel,checks,exactCounts,exactLocations,exactCoverage,exactTopLevel}
}
