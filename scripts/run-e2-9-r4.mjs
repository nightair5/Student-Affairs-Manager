...
const ENDPOINT = 'https://student-affairs-manager-preview.nightsdell.workers.dev/api/experiments/e2-9/r4/benchmark'
...
async function curlRequest(url, { token, body, method = 'POST' }) {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'e2-9-r4-curl-'))
...
async function assertMissing(file) {
  try { await readFile(file); throw new Error(`Fresh R4 label already has a checkpoint: ${path.relative(ROOT, file)}`) } catch (error) {
    if (error instanceof Error && error.message.startsWith('Fresh R4 label')) throw error
...
function validateGeneration(payload, observation) {
  ...
  if (!R4_COMPLETE_STATUSES.has(payload.protocolStatus)) return 'PROTOCOL_STATUS_NOT_COMPLETE'
  ...
  if (payload.execution.normalizer !== 'e2-9-r4-role-aware-normalizer-3.2.0') return 'NORMALIZER_VERSION_DRIFT'
...
async function runPhase(phase, token) {
  ...
  for (const observation of observations) {
    ...
    const checkpoint = {
      schemaVersion: 'e2.9-r4-checkpoint-3.2.0', protocolVersion: PROTOCOL_VERSION, runManifestSha256: run.runManifestSha256,
