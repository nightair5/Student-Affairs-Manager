...
const freeze = JSON.parse(await readFile(resolve(root, 'docs/recognition-optimization/RCO-5-008-B8-M1_RUNNER_FREEZE.json'), 'utf8'))
const sha = async (relativePath) => createHash('sha256').update(await readFile(resolve(root, relativePath))).digest('hex')

test('B8 M1 freezes the exact paid authorization and conservative budget', () => {
  assert.equal(freeze.authorizationId, 'RCO-5-008-B8-M1')
...
test('B8 M1 runner and pristine one-shot state match the freeze', async () => {
  ...
  assert.equal(await sha('docs/recognition-optimization/rco-5-008-b8-runs/rco-5-008-b8-m1-20260904a/checkpoint.json'), freeze.initialCheckpointSha256)
  const checkpoint = JSON.parse(await readFile(resolve(root, 'docs/recognition-optimization/rco-5-008-b8-runs/rco-5-008-b8-m1-20260904a/checkpoint.json'), 'utf8'))
  const raw = JSON.parse(await readFile(resolve(root, 'docs/recognition-optimization/rco-5-008-b8-runs/rco-5-008-b8-m1-20260904a/raw-results.json'), 'utf8'))
...
test('B8 M1 freeze keeps provider cost unobserved and release paths blocked', () => {
