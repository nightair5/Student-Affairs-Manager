...
export function validateUsageEnvelope(rawText, status, policy=BILLING_POLICY) {
  check(isDeepStrictEqual(policy,BILLING_POLICY)||isDeepStrictEqual(policy,FLASH41_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED06_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED07_POLICY)||isDeepStrictEqual(policy,FLASH41_PAIRED08_POLICY),'POLICY_CHANGED')
...
function replay(lines, manifestSha, manifest) {
  ...
  for (const [sequence, line] of lines.entries()) {
    ...
    if (sequence === 0) { exact(e, ['kind','manifestSha']); check(e.kind === 'init' && e.manifestSha === manifestSha, 'LEDGER_INITIAL') }
      ...
      exact(e,['kind','unitId','requestSha','candidateSha','nonce','reservedMicroCny'])
      const active=state.paired08??state.paired07??state.paired06??state.paired05??state.paired04??state.candidate03??state.candidate02??state.batch,g=active?.grant,u=g?.targets[state.reservations.length-(state.paired08?128:state.paired07?104:state.paired06?80:state.paired05?56:state.paired04?32:state.candidate03?24:state.candidate02?16:2)]
      ...
      check(state.reservations.length<g.maxTotalRequests
        &&state.reservations.reduce((n,r)=>n+r.costUpperMicroCny,0)+e.reservedMicroCny<=(state.paired08?FLASH41_PAIRED08_POLICY:BILLING_POLICY).limitMicroCny,'LIMIT')
      state.reservations.push({...e,status:'pending',costUpperMicroCny:e.reservedMicroCny})
    } else if (e.kind === 'batchSettle') {
      exact(e,['kind','unitId','nonce','requestSha','responseSha','responseId','usage','costUpperMicroCny'])
      const r=state.reservations.at(-1)
      const active=state.paired08??state.paired07??state.paired06??state.paired05??state.paired04??state.candidate03??state.candidate02??state.batch
      ...
      const policy=state.paired08?FLASH41_PAIRED08_POLICY:state.paired07?FLASH41_PAIRED07_POLICY:state.paired06?FLASH41_PAIRED06_POLICY:state.paired05?FLASH41_POLICY:BILLING_POLICY
...
function validateBatchGrant(input,manifest,manifestSha) {
  ...
  exact(g,['version','grantId','parentTail','parentSequence','ledgerPrefixBytes','ledgerPrefixSha','manifestSha',
    'bindingSha','head','sourcesSha','reviewSha','targets','billingEvidence','route','priorNonce','a02ResponseSha','maxTotalRequests',...(paired05||paired06||paired07||paired08?['policy']:[])])
  ...
  check((paired08||paired07||paired06||paired05||paired04||candidate03||candidate02||g.version==='real-input-batch-grant-1')&&/^[a-f0-9-]{36}$/.test(g.grantId)
    &&g.parentSequence===(paired08?264:paired07?215:paired06?166:paired05?117:paired04?68:candidate03?51:candidate02?34:5)&&g.maxTotalRequests===(paired08?168:paired07?128:paired06?104:paired05?80:paired04?56:candidate03?32:candidate02?24:16)&&integer(g.ledgerPrefixBytes,1048576)&&g.ledgerPrefixBytes>0
  ...
  if(paired04||paired05||paired06||paired07||paired08){
    const pairCount=paired08?20:12
    ...
    for(let i=0;i<pairCount;i++){
      const pair=g.targets.slice(i*2,i*2+2),prefix=paired08?pair[0]?.unitId?.slice(0,4):paired06||paired07?pair[0]?.unitId?.slice(0,4):`${paired05?'P':'N'}${String(i+1).padStart(2,'0')}-`
      if(paired08){check(/^(?:S(?:0[1-9]|1[0-2])|T0[1-8])-$/.test(prefix)&&!cases.has(prefix),'PAIRED08_CASE');cases.add(prefix);if(pair[0].unitId.endsWith('-03'))baselineFirst++}
      ...
      check(new Set(pair.map(u=>u.unitId)).size===2&&pair.some(u=>u.unitId===prefix+'03')&&pair.some(u=>u.unitId===prefix+(paired08?'08':paired07?'07':paired06?'06':paired05?'05':'04')),'PAIRED04_PAIR')
    ...
    check(candidates.get('03')!==candidates.get(paired08?'08':paired07?'07':paired06?'06':paired05?'05':'04'),'PAIRED04_SAME_CANDIDATE')
    if(paired06||paired07||paired08)check(cases.size===pairCount&&baselineFirst===pairCount/2,'PAIRED06_BALANCE')
...
async function batchBudget(dir,manifest,manifestSha,read,input) {
  ...
  const policy=paired08?FLASH41_PAIRED08_POLICY:paired07?FLASH41_PAIRED07_POLICY:paired06?FLASH41_PAIRED06_POLICY:paired05?FLASH41_POLICY:BILLING_POLICY
  const offset=paired08?128:paired07?104:paired06?80:paired05?56:paired04?32:candidate03?24:candidate02?16:2,newCandidate=paired08||paired07||paired06||paired05||paired04||candidate02||candidate03
  const active=s=>paired08?s.paired08:paired07?s.paired07:paired06?s.paired06:paired05?s.paired05:paired04?s.paired04:candidate03?s.candidate03:candidate02?s.candidate02:s.batch
  ...
  return Object.freeze({
    ...
    async reserve(unitId,requestText,requestedPolicy=policy,now=new Date().toISOString()) {
      const reservation=await locked(manifest.lockRoot,async()=>{
        ...
        if(!active(s)){await append(dir,manifest,s,{kind:paired08?'paired08Grant':paired07?'paired07Grant':paired06?'paired06Grant':paired05?'paired05Grant':paired04?'paired04Grant':candidate03?'candidate03Grant':candidate02?'candidate02Grant':'batchGrant',grant});s=await boundRead()}
