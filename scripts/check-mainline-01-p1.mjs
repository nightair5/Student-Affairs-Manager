...
const root = 'docs/recognition-optimization/mainline-01-p1/'
const baseline = JSON.parse(readFileSync(root + 'R2_BASELINE.json', 'utf8'))
const sha = (data) => createHash('sha256').update(data).digest('hex')
function protect() {
  const changed = Object.entries(baseline.hashes).filter(([path, hash]) =>
    !baseline.allowedExisting.includes(path) && sha(readFileSync(path)) !== hash).map(([path]) => path)
  const path = 'docs/recognition-optimization/OPTIMIZATION_LOG.md'
  const blob = execFileSync('git', ['show', baseline.head + ':' + path])
  const prefix = [blob, Buffer.from(blob.toString().replace(/\r?\n/g, '\r\n'))].find((data) => sha(data) === baseline.hashes[path])
  const appendOnly = Boolean(prefix && readFileSync(path).subarray(0, prefix.length).equals(prefix))
  const additions = [
    'src/domain/v2/confirmationV2.ts', 'src/domain/v2/confirmationV2.test.ts',
    'src/experiments/mainline01p1/confirmationHarness.ts', 'src/experiments/mainline01p1/confirmationHarness.test.ts',
    'src/experiments/mainline01p1/browser.tsx', 'scripts/serve-mainline-01-p1.mjs', 'scripts/check-mainline-01-p1.mjs',
  ]
  ...
  const result = { okay, checked: Object.keys(baseline.hashes).length - baseline.allowedExisting.length, changed, appendOnly, outOfScope }
  console.log(JSON.stringify(result))
  if (!okay) throw new Error('R2_PROTECTION_FAILED')
  return result
}
...
protect()
const directory = mkdtempSync(join(tmpdir(), 'rco-mainline-01-p1-r2-'))
...
const tests = ['node_modules/vitest/vitest.mjs', 'run',
  ...(targeted ? ['src/domain/v2/confirmationV2.test.ts', 'src/experiments/mainline01p1/confirmationHarness.test.ts', 'src/experiments/mainline01'] : []),
