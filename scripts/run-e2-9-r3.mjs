...
import { assertCanonicalBinding, assertRunManifestBinding, deriveCheckpointGateStatus, R3_COMPLETE_STATUSES } from './e2-9-r3-integrity.mjs'
...
const ENDPOINT = 'https://student-affairs-manager-preview.nightsdell.workers.dev/api/experiments/e2-9/r3/benchmark'
...
async function curlRequest(url, { token, body, method = 'POST' }) {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'e2-9-r3-curl-'))
...
async function assertMissing(file) {
  try { await readFile(file); throw new Error(`Fresh R3 label already has a checkpoint: ${path.relative(ROOT, file)}`) } catch (error) {
    if (error instanceof Error && error.message.startsWith('Fresh R3 label')) throw error
...
async function loadInputs() {
  ...
  if (activation.runManifestSha256 !== run.runManifestSha256 || activation.protocolBundleSha256 !== bundle.bundles.protocolAndDeployment.sha256) throw new Error('ACTIVATION_BINDING_MISMATCH')
...
function validateGeneration(payload, observation) {
  ...
  if (!R3_COMPLETE_STATUSES.has(payload.protocolStatus)) return 'PROTOCOL_STATUS_NOT_COMPLETE'
  ...
  if (payload.execution.normalizer !== 'e2-9-r3-role-aware-normalizer-3.1.0') return 'NORMALIZER_VERSION_DRIFT'
...
async function runPhase(phase, token) {
  ...
  for (const observation of observations) {
    ...
    const checkpoint = {
      schemaVersion: 'e2.9-r3-checkpoint-3.1.0', protocolVersion: PROTOCOL_VERSION, runManifestSha256: run.runManifestSha256,
