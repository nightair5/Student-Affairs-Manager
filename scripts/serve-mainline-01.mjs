...
const bundled = await build({ entryPoints: ['src/experiments/mainline01/browser.tsx'], bundle: true,
  write: false, platform: 'browser', format: 'esm', jsx: 'automatic',
  define: { 'process.env.NODE_ENV': '"test"' } })
const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>MAINLINE-01 隔离工程验证</title>
...
server.listen(0, '127.0.0.1', () => console.log(`MAINLINE_TEST_URL=http://127.0.0.1:${server.address().port}/?run=${randomUUID()}`))
