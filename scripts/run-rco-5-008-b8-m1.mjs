...
import { composeLocalAnchorsV2 } from '../src/recognition/modelAnchorLocalComposerV2.ts'
import { SCOPE_INDEX_VERSION, indexImmutableScopesV11 } from '../src/recognition/scopeIndexV11.ts'
import { formLocalTaskSuggestionsP4, validateLocalTaskFormationP4 } from '../src/recognition/taskFormationPolicyP4.ts'
import { aggregateTaskFormationScores, scoreTaskFormationCase } from '../src/recognition/taskFormationEvaluation.ts'
import { materializeRevisionRelationsByScope, scoreStableDefaultSafety } from '../src/recognition/taskFormationEvaluationV2.ts'

const ROOT = process.cwd()
const AUTHORIZATION_ID = 'RCO-5-008-B8-M1'
const RUN_ID = 'rco-5-008-b8-m1-20260904a'
...
const DATASET_PATH = 'docs/recognition-optimization/RCO-5-008-B8_DEVELOPMENT_DATASET.json'
const DATA_FREEZE_PATH = 'docs/recognition-optimization/RCO-5-008-B8_DATA_FREEZE.json'
const RUNNER_FREEZE_PATH = 'docs/recognition-optimization/RCO-5-008-B8-M1_RUNNER_FREEZE.json'
const RUNNER_PATH = 'scripts/run-rco-5-008-b8-m1.mjs'
const RUN_DIR = `docs/recognition-optimization/rco-5-008-b8-runs/${RUN_ID}`
...
function predictionFromP4(fixture, index, formed, status) {
...
async function buildRequest(fixture) {
  ...
  const body = {
    ...
    text: { format: { type: 'json_schema', name: 'rco_b8_anchor_selection', schema: MODEL_ANCHOR_SELECTION_JSON_SCHEMA } },
  }
...
async function dispatchOnce(apiKey, fixture, checkpoint, records) {
  ...
  try {
    ...
    records.push(record)
    await atomicJson(RAW_RESULTS_PATH, { schemaVersion: 'rco-5-008-b8-m1-raw-1.0.0', runId: RUN_ID, records })
    await atomicJson(CHECKPOINT_PATH, checkpoint)
    return record
  }
  clearTimeout(timeout)
...
async function evaluate(dataset, records) {
  ...
  for (const fixture of dataset.cases) {
    ...
    let actionNormalizations = []
    let conditionAttachments = []
    let status = 'invalid'
    if (selection) {
      const composition = composeLocalAnchorsV2(selection, index, `candidate-${RUN_ID}-${fixture.id}`)
      if (composition.ok) {
        composerWarnings = composition.value.warnings.map((issue) => `${issue.code}:${issue.path}`)
        actionNormalizations = composition.value.actionNormalizations
        conditionAttachments = composition.value.conditionAttachments
        formed = formLocalTaskSuggestionsP4(index, composition.value.reduced)
        contractIssues = validateLocalTaskFormationP4(formed, index, composition.value.reduced).map((issue) => `${issue.code}:${issue.path}`)
        status = contractIssues.length === 0 ? 'completed' : 'invalid'
      } else {
        composerIssues = composition.issues.map((issue) => `${issue.code}:${issue.path}`)
        contractIssues = ['LOCAL_COMPOSITION_INVALID']
      }
    }
    const prediction = predictionFromP4(fixture, index, formed, status)
    const taskScore = scoreTaskFormationCase(fixture, prediction)
    const safetyScore = scoreStableDefaultSafety(fixture.expected.directives, prediction.tasks)
    const relationScore = status === 'completed'
      ? materializeRevisionRelationsByScope(fixture.expected.directives, fixture.expected.revisionRelations, prediction.tasks, formed, index)
      : { expected: fixture.expected.revisionRelations, actual: [], exact: fixture.expected.revisionRelations.length === 0 }
    const scopeById = new Map(index.scopes.map((scope) => [scope.id, scope.text]))
    const unresolved = status === 'completed' ? formed.unresolvedRevisionScopeIds.map((id) => scopeById.get(id) ?? null) : []
    cases.push({ caseId: fixture.id, coverageTags: fixture.coverageTags, modelStatus: record?.status ?? 'not_run', schemaIssues: record?.schemaIssues ?? ['NOT_RUN'], anchorScore, composerIssues, composerWarnings, actionNormalizations, conditionAttachments, contractIssues, prediction, taskScore, safetyScore, relationScore, expectedRelations: fixture.expected.revisionRelations, actualRelations: relationScore.actual, relationExact: relationScore.exact, expectedUnresolvedScopeTexts: fixture.expected.unresolvedRevisionScopeTexts, actualUnresolvedScopeTexts: unresolved, unresolvedExact: arraysEqual(unresolved, fixture.expected.unresolvedRevisionScopeTexts) })
  }
  ...
  const findTask = (key) => {
    ...
    return directive ? cases.find((item) => item.caseId === caseId)?.prediction.tasks.find((task) => task.object === directive.object.surface && task.propositionScopeTexts.some((text) => directive.propositionScopeTexts.includes(text))) : undefined
  }
  ...
  return { cases, anchorMetrics, taskMetrics, unsafeDefaultFalsePositives, revisionMetrics }
}
...
async function execute(apiKey) {
  ...
  const result = {
    schemaVersion: 'rco-5-008-b8-m1-score-1.0.0', runId: RUN_ID, authorizationId: AUTHORIZATION_ID, generatedAt: new Date().toISOString(), runStatus: checkpoint.status, stopReason,
...
async function main() {
  ...
  const apiKey = String(process.env.RCO_B8_DEEPSEEK_API_KEY ?? '').trim()
  if (!apiKey) throw new Error('RCO_B8_DEEPSEEK_API_KEY_REQUIRED')
...
main().catch((error) => { console.error(`RCO_5_008_B8_M1_FAILED:${error instanceof Error ? error.message : 'UNKNOWN'}`); process.exitCode = 1 })
