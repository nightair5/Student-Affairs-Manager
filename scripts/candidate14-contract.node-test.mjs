...
const reference=()=>({version:CANDIDATE14_REFERENCE_VERSION,sourceId:'S',completeness:'complete',allowedAliases:[],tasks:[task()],relations:[],noTaskFacts:[]})
...
test('ambiguous maximum matching requires adjudication',()=>{const ref=reference();ref.tasks.push(task('e2'));const score=scoreCandidate14(ref,{tasks:[actual({id:'a1'}),actual({id:'a2'})]});assert.equal(score.status,'ADJUDICATION_REQUIRED')})
