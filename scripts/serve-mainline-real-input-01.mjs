...
export function loadRecordedCandidate02() {
  loadRecordedBatch() // verifies the whole appended chain and historical prefix
  const root='docs/recognition-optimization/mainline-real-input-01/runs/',D=root+'candidate02-20260908a/'
  const bytes=readFileSync(D+'BINDING.json'),binding=JSON.parse(bytes)
  const records=readFileSync(root+'usage-resume-20260907a/CALL_LEDGER.jsonl','utf8').trimEnd().split('\n').map(JSON.parse)
  const grants=records.filter(r=>r.event.kind==='candidate02Grant')
  check(grants.length===1&&grants[0].event.grant.bindingSha===hash(bytes),'C02_GRANT_BINDING')
  const original=loadRecordedA02()
  return binding.units.map((u,i)=>{
    const raw=JSON.parse(readFileSync(D+u.unitId+'_RAW.jsonl')),result=JSON.parse(readFileSync(D+u.unitId+'_RESULT.json'))
    const settlements=records.filter(r=>r.event.kind==='batchSettle'&&r.event.unitId===u.unitId)
    check(u.unitId===`C0${i+1}`&&raw.unitId===u.unitId&&raw.httpStatus===200&&result.lastHttpStatus===200&&!result.stopDispatch
      &&hash(raw.rawHttpText)===raw.responseSha&&raw.responseSha===result.responseSha&&result.bindingSha===hash(bytes)
      &&raw.requestSha===u.requestSha&&raw.candidateSha===u.candidateSha&&raw.inputSha===u.inputSha
      &&settlements.length===1&&settlements[0].event.requestSha===u.requestSha&&settlements[0].event.responseSha===raw.responseSha,'C02_RECORDED_BINDING')
    return {version:'recorded-candidate02-1',unitId:u.unitId,name:original.name,handle:binding.items[i].handle,
      context:binding.items[i].context,requestSha:u.requestSha,responseSha:raw.responseSha,rawHttpText:raw.rawHttpText}
  })
}
export function loadRecordedCandidate03() {
  loadRecordedCandidate02() // Retain the original chain and both previous batch bindings.
  const root='docs/recognition-optimization/mainline-real-input-01/runs/',D=root+'candidate03-20260908a/'
  const bytes=readFileSync(D+'BINDING.json'),binding=JSON.parse(bytes)
  const records=readFileSync(root+'usage-resume-20260907a/CALL_LEDGER.jsonl','utf8').trimEnd().split('\n').map(JSON.parse)
  const grants=records.filter(r=>r.event.kind==='candidate03Grant')
  check(grants.length===1&&grants[0].event.grant.bindingSha===hash(bytes),'C03_GRANT_BINDING')
  const original=loadRecordedA02()
  return binding.units.map((u,i)=>{
    const raw=JSON.parse(readFileSync(D+u.unitId+'_DISPATCH_RAW.jsonl')),result=JSON.parse(readFileSync(D+u.unitId+'_DISPATCH_RESULT.json'))
    const settlements=records.filter(r=>r.event.kind==='batchSettle'&&r.event.unitId===u.unitId)
    check(u.unitId===`D0${i+1}`&&raw.unitId===u.unitId&&raw.httpStatus===200&&result.lastHttpStatus===200&&!result.stopDispatch
      &&hash(raw.rawHttpText)===raw.responseSha&&raw.responseSha===result.responseSha&&result.bindingSha===hash(bytes)
      &&raw.requestSha===u.requestSha&&raw.candidateSha===u.candidateSha&&raw.inputSha===u.inputSha
      &&settlements.length===1&&settlements[0].event.requestSha===u.requestSha&&settlements[0].event.responseSha===raw.responseSha,'C03_RECORDED_BINDING')
    return {version:'recorded-candidate03-1',unitId:u.unitId,name:original.name,handle:binding.items[i].handle,
      context:binding.items[i].context,requestSha:u.requestSha,responseSha:raw.responseSha,rawHttpText:raw.rawHttpText}
  })
}
export function loadRecordedPaired04() {
  loadRecordedCandidate03()
  const root='docs/recognition-optimization/mainline-real-input-01/runs/',D=root+'candidate04-20260909a/'
  const bytes=readFileSync(D+'BINDING.json'),binding=JSON.parse(bytes)
  const rows=readFileSync(root+'usage-resume-20260907a/CALL_LEDGER.jsonl','utf8').trimEnd().split('\n').map(JSON.parse)
  const grants=rows.filter(r=>r.event.kind==='paired04Grant')
  check(grants.length===1&&grants[0].event.grant.bindingSha===hash(bytes),'P04_GRANT')
  return binding.units.map(u=>{
    const raw=JSON.parse(readFileSync(D+u.unitId+'_RAW.jsonl')),result=JSON.parse(readFileSync(D+u.unitId+'_RESULT.json'))
    const settled=rows.filter(r=>r.event.kind==='batchSettle'&&r.event.unitId===u.unitId),item=binding.items.find(i=>i.id===u.unitId.slice(0,3))
    check(item&&raw.unitId===u.unitId&&raw.httpStatus===200&&result.http===200&&!result.stopDispatch
      &&hash(raw.rawHttpText)===raw.responseSha&&raw.responseSha===result.responseSha&&result.bindingSha===hash(bytes)
      &&raw.requestSha===u.requestSha&&hash(binding.requests[u.unitId])===u.requestSha
      &&raw.candidateSha===u.candidateSha&&raw.inputSha===u.inputSha&&settled.length===1
      &&settled[0].event.requestSha===u.requestSha&&settled[0].event.responseSha===raw.responseSha,'P04_RECORDED_BINDING')
    return {version:'recorded-paired04-1',unitId:u.unitId,name:loadRecordedA02().name,operationId:item.operationId,title:item.title,
...
export async function createLocalApp({port,carrierManifest,checking=false,recordedA02=false,recordedBatch=false,recordedCandidate02=false,recordedCandidate03=false,recordedPaired04=false,recordedPaired05=false,recordedPaired06=false}) {
  ...
  const evidence={mode,realApp:true,upstreamEnabled:false,carriers:recorded||(batch&&!recordedCandidate02)?0:carriers.length,forbiddenBrowserInputs:forbidden,
