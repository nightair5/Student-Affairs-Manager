...
export class E2R3RunLedger {
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/register' && request.method === 'POST') {
      ...
      const created = {
        schemaVersion: 'e2.9-r3-run-ledger-3.1.0', protocolVersion: PROTOCOL_VERSION,
...
export default {
  async fetch(request, env) {
    const runLabel = request.headers.get('x-e2-r3-run-label') ?? ''
    if (!/^[a-z0-9][a-z0-9._-]{2,100}$/u.test(runLabel)) return json({ error: 'INVALID_RUN_LABEL' }, 400)
    const id = env.E2_R3_RUN_LEDGER.idFromName(runLabel)
    return env.E2_R3_RUN_LEDGER.get(id).fetch(request)
  },
}
