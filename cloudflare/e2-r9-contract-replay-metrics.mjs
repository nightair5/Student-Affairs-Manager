...
function normalize(value) {
  return typeof value === 'string'
    ? value.toLowerCase().replace(/[\s\p{P}\p{S}和及与并的]/gu, '')
    : ''
}
...
function conditionRepresents(resultAmbiguities, condition) {
  return array(resultAmbiguities).some((value) => {
    ...
    return Boolean(message && conditionText && (message.includes(conditionText) || conditionText.includes(message)))
  })
}
...
export function evaluateR9ContractCoverage(resultValue, graph) {
  ...
  const timeRolesCorrect = graph.timePoints.filter((timePoint) => array(result.timePoints).some((value) => {
    const item = record(value)
    return (item.tempId === timePoint.sourceTimePointId || overlaps(item.rawText, timePoint.rawText)) && item.type === timePoint.role
  })).length
  ...
  const coveredFacts = obligationsCovered + materialsCovered + timesCovered + eventsCovered + conditionsCovered + ambiguitiesCovered
  return {
    counts: {
...
export function aggregateR9ContractCoverage(values) {
  const names = [
  ...
  const counts = Object.fromEntries(names.map((name) => [name, values.reduce((sum, value) => sum + value.counts[name], 0)]))
  return {
    counts,
    rates: {
...
export function assertR9PlannedResultContract(result, graph) {
  const coverage = evaluateR9ContractCoverage(result, graph)
  const issues = []
  ...
  if (issues.length) throw new Error(`R9_PLANNED_RESULT_CONTRACT_INVALID:${issues.join(',')}`)
  return result
}
