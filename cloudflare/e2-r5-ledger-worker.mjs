...
const REGISTRATION_FIELDS = new Set(['runLabel', 'protocolVersion', 'runManifestSha256', 'bindings', 'observations'])
const REGISTRATION_BINDING_FIELDS = new Set([
  'sourceOnlySha256', 'readinessManifestSha256', 'smokeManifestSha256', 'screeningManifestSha256',
  'bundleManifestSha256', 'protocolBundleSha256', 'promptAndPipelineSha256', 'schemaBundleSha256',
  'scorerSemanticsSha256', 'datasetBundleSha256', 'activationSha256', 'deploymentVersion', 'ledgerDeploymentVersion',
])
const OBSERVATION_PLAN_FIELDS = new Set([
  'observationId', 'phase', 'caseAlias', 'caseId', 'probeIndex', 'modelAlias', 'semanticRole',
  'sourceSha256', 'inputSha256', 'phaseManifestSha256', 'maxAttempts',
])
...
function pairedPhaseValid(items, expectedCases) {
  if (items.length !== expectedCases * 2) return false
  const byCase = new Map()
  for (const item of items) {
    if (typeof item.caseId !== 'string' || !item.caseId || !SEMANTIC_ROLES.has(item.semanticRole) || !validSha256(item.sourceSha256)) return false
    const aliases = byCase.get(item.caseId) ?? new Set()
    aliases.add(item.modelAlias)
    byCase.set(item.caseId, aliases)
  }
  return byCase.size === expectedCases && [...byCase.values()].every((aliases) => aliases.size === 2 && aliases.has('flash') && aliases.has('pro'))
}
...
export class E2R5RunLedger {
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/register' && request.method === 'POST') {
      ...
      if (!onlyFields(body, REGISTRATION_FIELDS)) return json({ error: 'INVALID_REGISTRATION_FIELDS' }, 400)
      if (body.protocolVersion !== PROTOCOL_VERSION || !/^[a-z0-9][a-z0-9._-]{2,100}$/u.test(body.runLabel ?? '')
        || !validSha256(body.runManifestSha256) || !Array.isArray(body.observations)) return json({ error: 'INVALID_REGISTRATION_IDENTITY' }, 400)
      if (!onlyFields(body.bindings, REGISTRATION_BINDING_FIELDS)
        || ![...REGISTRATION_BINDING_FIELDS].every((key) => (key === 'deploymentVersion' || key === 'ledgerDeploymentVersion')
          ? typeof body.bindings[key] === 'string' && body.bindings[key].length >= 16
          : validSha256(body.bindings[key]))) return json({ error: 'INVALID_REGISTRATION_BINDINGS' }, 400)
      const ids = new Set()
      ...
      for (const item of body.observations) {
        ...
        if (item.phase === 'readiness') {
          if (!Number.isInteger(item.probeIndex) || item.probeIndex < 1 || item.probeIndex > 3 || item.caseId !== null
            || item.semanticRole !== null || item.sourceSha256 !== null) return json({ error: 'INVALID_READINESS_PLAN' }, 400)
        } else if (typeof item.caseId !== 'string' || !item.caseId || !SEMANTIC_ROLES.has(item.semanticRole) || !validSha256(item.sourceSha256)) {
          return json({ error: 'INVALID_GENERATION_PLAN' }, 400)
        }
        ...
        expectedObservations[item.observationId] = {
          phase: item.phase,
          caseId: item.caseId,
          probeIndex: item.probeIndex ?? null,
      ...
      const readiness = body.observations.filter((item) => item.phase === 'readiness')
      if (!['flash', 'pro'].every((alias) => {
        const probes = readiness.filter((item) => item.modelAlias === alias).map((item) => item.probeIndex).sort()
        return probes.length === 3 && probes.join(',') === '1,2,3'
      }) || !pairedPhaseValid(body.observations.filter((item) => item.phase === 'smoke'), 5)
        || !pairedPhaseValid(body.observations.filter((item) => item.phase === 'screening'), 8)) return json({ error: 'PAIRED_OBSERVATION_PLAN_INVALID' }, 400)
      const created = {
        schemaVersion: 'e2.9-r5-run-ledger-3.3.0', protocolVersion: PROTOCOL_VERSION,
    ...
    if (url.pathname === '/reserve' && request.method === 'POST') {
      ...
      if (expected.modelAlias !== body.modelAlias || expected.inputSha256 !== body.inputSha256 || expected.phaseManifestSha256 !== body.phaseManifestSha256
        || (phase === 'readiness' && expected.probeIndex !== body.probeIndex)
        || (phase !== 'readiness' && (expected.caseId !== body.caseId || expected.semanticRole !== body.semanticRole || expected.sourceSha256 !== body.sourceSha256))) return json({ error: 'OBSERVATION_BINDING_MISMATCH' }, 412)
      ...
      run.observations[observationId] = {
        ...
        reservedAt: new Date().toISOString(), reservationToken, caseId: expected.caseId, probeIndex: expected.probeIndex,
        sourceSha256: expected.sourceSha256, inputSha256: body.inputSha256, phaseManifestSha256: body.phaseManifestSha256,
    ...
    if (url.pathname === '/finalize' && request.method === 'POST') {
      ...
      if (TERMINAL_COMPLETE.has(status)) {
        const expectedModel = MODEL_BY_ALIAS[record.modelAlias]
        if (![body.requestedModel, body.returnedModel, body.executionModel, body.resultModelName].every((value) => value === expectedModel)) return json({ error: 'FINAL_MODEL_LINEAGE_MISMATCH' }, 412)
        if (record.phase !== 'readiness' && body.sourceSha256 !== record.sourceSha256) return json({ error: 'FINAL_SOURCE_BINDING_MISMATCH' }, 412)
      }
      record.state = 'final'
...
export default {
  async fetch(request, env) {
    const runLabel = request.headers.get('x-e2-r5-run-label') ?? ''
    if (!/^[a-z0-9][a-z0-9._-]{2,100}$/u.test(runLabel)) return json({ error: 'INVALID_RUN_LABEL' }, 400)
    const id = env.E2_R5_RUN_LEDGER.idFromName(runLabel)
    return env.E2_R5_RUN_LEDGER.get(id).fetch(request)
  },
}
