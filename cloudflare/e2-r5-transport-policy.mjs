export const E2_R5_TRANSPORT_POLICY_VERSION = 'e2-9-r5-transport-policy-3.3.0'
...
export async function classifyR5Failure(error, execution, expectedModel) {
  ...
  const returnedModel = typeof execution?.returnedModel === 'string' && execution.returnedModel.trim()
    ? execution.returnedModel.trim().slice(0, 100)
    : null
  const observedModel = rawResponse === null ? returnedModel : (extractEnvelopeModel(rawResponse) ?? returnedModel)
  ...
  else if (error === 'MODEL_FALLBACK_DETECTED' && !observedModel) classification = 'MODEL_IDENTITY_UNVERIFIABLE'
  ...
  return {
    schemaVersion: E2_R5_TRANSPORT_POLICY_VERSION,
    classification,
...
export function attemptStatusForFailure(evidence) {
  ...
  if (evidence.classification === 'MODEL_IDENTITY_FAILURE' || evidence.classification === 'MODEL_IDENTITY_UNVERIFIABLE') return 'integrity_failure'
