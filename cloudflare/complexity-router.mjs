...
function matchCount(content, pattern) { return content.match(pattern)?.length ?? 0 }

export function extractRecognitionComplexityFeatures(content) {
...
export function routeRecognitionSource(content, enableIntensiveSinglePass = false) {
  ...
  if (features.hasInformationOnlyCue && !features.hasPromptInjection && !features.hasOptionalOrReferenceBoundary && features.conditionCueCount === 0) {
    level = 'simple'; reasons.push('明确的信息知悉或无需操作语义')
  } else {
    ...
    if (complexSignals.some(Boolean)) {
      ...
      const mediumSignals = [features.hasPromptInjection, features.hasOcrNoise, features.hasAmbiguity, features.conditionCueCount >= 1, features.actionCueCount >= 2, features.eventCueCount >= 1 && features.actionCueCount >= 2, features.hasOptionalOrReferenceBoundary && features.materialCueCount >= 1, features.hasConstraintBundle, CORRECTION_OR_SUPPLEMENT.test(content), features.hasCrossParagraphReference, features.channelCueCount >= 2, features.pdfPageCueCount >= 2]
  ...
  const candidateStrategy = level === 'complex' ? 'intensive_single_pass' : 'single_pass'
  const intensiveModeEnabled = candidateStrategy === 'intensive_single_pass' && enableIntensiveSinglePass
  return { routerVersion: RECOGNITION_ROUTER_VERSION, level, score, reasons, features, candidateStrategy, selectedStrategy: intensiveModeEnabled ? 'intensive_single_pass' : 'single_pass', intensiveModeEnabled }
}
