...
import { attemptStatusForFailure, classifyR4Failure } from './e2-r4-transport-policy.mjs'

export const E2_R4_PROTOCOL_VERSION = 'e2-9-v4-pro-protocol-3.2.0'
export const E2_R4_BENCHMARK_VERSION = 'e2-9-r4-benchmark-3.2.0'
export const E2_R4_NORMALIZER_VERSION = 'e2-9-r4-role-aware-normalizer-3.2.0'
...
function authorizationError(request, env) {
  ...
  const expectedOrigin = safeText(env.E2_R4_PREVIEW_ORIGIN, 300)
  if (env.E2_R4_BENCHMARK_ENABLED !== 'true' || !expectedOrigin || url.origin !== expectedOrigin) return 'NOT_FOUND'
  ...
  if (!env.E2_R4_LEDGER || typeof env.E2_R4_LEDGER.fetch !== 'function') return 'LEDGER_NOT_CONFIGURED'
  return null
}

async function ledgerRequest(env, runLabel, path, { method = 'POST', body } = {}) {
  const response = await env.E2_R4_LEDGER.fetch(`https://e2-r4-ledger.internal${path}`, {
    method,
    headers: { 'content-type': 'application/json', 'x-e2-r4-run-label': runLabel },
...
function sanitizedFailure(error, body, protocolAttempts, evidence, protocolStatus) {
  return {
    error,
    protocolVersion: E2_R4_PROTOCOL_VERSION,
    benchmarkVersion: E2_R4_BENCHMARK_VERSION,
...
async function handleReadiness(request, env, fetcher) {
  ...
  if (body.protocolVersion !== E2_R4_PROTOCOL_VERSION || !Object.hasOwn(MODEL_BY_ALIAS, body.modelAlias)
  ...
  try {
    ...
    if (!complete) {
      ...
      evidence = await classifyR4Failure(error, payload?.execution, expected)
    ...
    return json({
      ...payload, protocolVersion: E2_R4_PROTOCOL_VERSION, benchmarkVersion: E2_R4_BENCHMARK_VERSION,
...
async function handleGenerate(request, env, fetcher) {
  ...
  if (body.protocolVersion !== E2_R4_PROTOCOL_VERSION || !['smoke', 'screening'].includes(body.phase)
  ...
  try {
    for (let attemptNumber = 1; attemptNumber <= 2; attemptNumber += 1) {
      ...
      if (!upstream.ok || !payload?.result || !payload?.execution) {
        ...
        const evidence = await classifyR4Failure(upstreamError, payload?.execution, expectedModel)
      ...
      const responsePayload = {
        ...
        result,
        execution: { ...payload.execution, executionModel, semanticRole: body.semanticRole, normalizer: E2_R4_NORMALIZER_VERSION, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200) },
      }
...
export async function runE2R4Benchmark(request, env, fetcher = fetch) {
  ...
  if (url.pathname.endsWith('/register') && request.method === 'POST') {
    ...
    if (body?.protocolVersion !== E2_R4_PROTOCOL_VERSION) return json({ error: 'PROTOCOL_VERSION_MISMATCH' }, 412)
