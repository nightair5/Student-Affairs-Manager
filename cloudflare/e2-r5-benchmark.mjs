...
import { attemptStatusForFailure, classifyR5Failure } from './e2-r5-transport-policy.mjs'

export const E2_R5_PROTOCOL_VERSION = 'e2-9-v4-pro-protocol-3.3.0'
export const E2_R5_BENCHMARK_VERSION = 'e2-9-r5-benchmark-3.3.0'
export const E2_R5_NORMALIZER_VERSION = 'e2-9-r5-role-aware-normalizer-3.3.0'
...
const GENERATE_FIELDS = new Set([
  'runLabel', 'observationId', 'phase', 'caseId', 'modelAlias', 'semanticRole', 'sourceType', 'sourceTitle', 'content',
...
function authorizationError(request, env) {
  ...
  const expectedOrigin = safeText(env.E2_R5_PREVIEW_ORIGIN, 300)
  if (env.E2_R5_BENCHMARK_ENABLED !== 'true' || !expectedOrigin || url.origin !== expectedOrigin) return 'NOT_FOUND'
  if (request.headers.get('origin') !== expectedOrigin) return 'ORIGIN_NOT_ALLOWED'
  const expected = safeText(env.E2_R5_BENCHMARK_TOKEN, 512)
  ...
  if (!env.E2_R5_LEDGER || typeof env.E2_R5_LEDGER.fetch !== 'function') return 'LEDGER_NOT_CONFIGURED'
  return null
}

async function ledgerRequest(env, runLabel, path, { method = 'POST', body } = {}) {
  const response = await env.E2_R5_LEDGER.fetch(`https://e2-r5-ledger.internal${path}`, {
    method,
    headers: { 'content-type': 'application/json', 'x-e2-r5-run-label': runLabel },
...
async function reserve(env, body) {
  return ledgerRequest(env, body.runLabel, '/reserve', { body: {
    ...
    caseId: body.caseId ?? null, probeIndex: body.probeIndex ?? null, semanticRole: body.semanticRole ?? null,
...
function r1Environment(env) {
  return {
    ...
    E2_V4_PRO_BENCHMARK_TOKEN: env.E2_R5_BENCHMARK_TOKEN,
  }
}
...
function sanitizedFailure(error, body, protocolAttempts, evidence, protocolStatus) {
  return {
    ...
    benchmarkVersion: E2_R5_BENCHMARK_VERSION,
...
async function handleReadiness(request, env, fetcher) {
  ...
  if (body.protocolVersion !== E2_R5_PROTOCOL_VERSION || !Object.hasOwn(MODEL_BY_ALIAS, body.modelAlias)
  ...
  try {
    ...
    if (!complete) {
      ...
      evidence = await classifyR5Failure(error, payload?.execution, expected)
    ...
    return json({
      ...payload, protocolVersion: E2_R5_PROTOCOL_VERSION, benchmarkVersion: E2_R5_BENCHMARK_VERSION,
    ...
    try {
      if (!attemptRecorded) await appendAttempt(env, body, token, { attemptNumber: 1, status: 'integrity_failure', error: 'HARNESS_FAILURE', requestedModel: expected, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200) })
      await finalize(env, body, token, { requestedModel: expected, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200) })
    } catch { /* ledger unavailability is already a fail-closed terminal response */ }
    return json({ error: 'HARNESS_FAILURE', observationId: body.observationId, protocolStatus: 'integrity_failure' }, 502)
  }
}
...
async function handleGenerate(request, env, fetcher) {
  ...
  if (body.protocolVersion !== E2_R5_PROTOCOL_VERSION || !['smoke', 'screening'].includes(body.phase) || typeof body.caseId !== 'string' || !body.caseId
  ...
  try {
    for (let attemptNumber = 1; attemptNumber <= 2; attemptNumber += 1) {
      ...
      if (!upstream.ok || !payload?.result || !payload?.execution) {
        ...
        const evidence = await classifyR5Failure(upstreamError, payload?.execution, expectedModel)
        ...
        const error = evidence.classification === 'UPSTREAM_JSON_TRUNCATED' ? 'UPSTREAM_JSON_TRUNCATED'
          : evidence.classification === 'MODEL_JSON_INVALID' ? 'MODEL_JSON_INVALID'
            : evidence.classification === 'MODEL_IDENTITY_UNVERIFIABLE' ? 'MODEL_IDENTITY_UNVERIFIABLE'
              : upstreamError
      ...
      const responsePayload = {
        ...
        result,
        execution: { ...payload.execution, executionModel, semanticRole: body.semanticRole, normalizer: E2_R5_NORMALIZER_VERSION, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200) },
      }
    ...
    try {
      if (recordedAttemptNumber < currentAttemptNumber) {
        await appendAttempt(env, body, token, {
          attemptNumber: currentAttemptNumber || 1, status: 'integrity_failure', error: 'HARNESS_FAILURE',
          requestedModel: expectedModel, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200),
        })
      }
      await finalize(env, body, token, { sourceSha256: body.sourceSha256, requestedModel: expectedModel, deploymentVersion: safeText(env.CF_VERSION_METADATA?.id, 200) })
    } catch { /* ledger unavailability is already a fail-closed terminal response */ }
    return json({ error: 'HARNESS_FAILURE', observationId: body.observationId, protocolStatus: 'integrity_failure' }, 502)
  }
}

export async function runE2R5Benchmark(request, env, fetcher = fetch) {
  ...
  if (url.pathname.endsWith('/register') && request.method === 'POST') {
    ...
    if (body?.protocolVersion !== E2_R5_PROTOCOL_VERSION) return json({ error: 'PROTOCOL_VERSION_MISMATCH' }, 412)
