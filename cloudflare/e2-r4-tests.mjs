...
import { E2R4RunLedger } from './e2-r4-ledger-worker.mjs'
import { E2_R4_PROTOCOL_VERSION, runE2R4Benchmark } from './e2-r4-benchmark.mjs'
import { classifyR4Failure } from './e2-r4-transport-policy.mjs'
...
function ledgerService() {
  ...
  return {
    ...
    async fetch(url, init) {
      ...
      const label = request.headers.get('x-e2-r4-run-label')
      if (!instances.has(label)) instances.set(label, new E2R4RunLedger({ storage: new MemoryStorage() }))
      return instances.get(label).fetch(request)
    },
  }
}

const TOKEN = 'r4-test-token-that-is-longer-than-thirty-two-characters'
...
function environment(ledger) {
  return {
    E2_R4_BENCHMARK_ENABLED: 'true',
    E2_R4_PREVIEW_ORIGIN: 'https://preview.example.test',
    E2_V4_PRO_BENCHMARK_TOKEN: TOKEN,
    DEEPSEEK_API_KEY: 'server-only-test-key-for-e2-r4',
    E2_R4_LEDGER: ledger,
...
function buildRegistration(runLabel, target) {
  ...
  return { runLabel, protocolVersion: E2_R4_PROTOCOL_VERSION, runManifestSha256: HASH, bindings: { protocolBundleSha256: HASH }, observations: [...readiness, ...smoke, ...screening] }
}
...
async function primeSmoke({ runLabel, modelAlias = 'flash', semanticRole = 'information_only' }) {
  ...
  const registered = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/register', registration), env)
  ...
  const body = {
    ...
    phaseManifestSha256: HASH, protocolVersion: E2_R4_PROTOCOL_VERSION,
  }
...
test('truncation classification requires machine evidence and exact model identity', async () => {
  ...
  const evidence = await classifyR4Failure('UPSTREAM_JSON_INVALID', {
    rawResponse,
  ...
  const wrongModel = await classifyR4Failure('UPSTREAM_JSON_INVALID', {
    rawResponse,
    attempts: [{ attempt: 1, status: 200, transportStatus: 'response_received' }],
    upstreamHeaders: { 'content-type': 'application/json' },
  }, 'deepseek-v4-pro')
  ...
  const malformed = await classifyR4Failure('UPSTREAM_JSON_INVALID', {
...
test('T01 first truncated and second valid becomes complete_after_protocol_retry', async () => {
  const { ledger, env, body } = await primeSmoke({ runLabel: 'r4-t01' })
  let calls = 0
  ...
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, fetcher)
...
test('T02 two truncated attempts become transport_integrity_failure', async () => {
  const { ledger, env, body } = await primeSmoke({ runLabel: 'r4-t02' })
  let calls = 0
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async (_url, init) => {
    calls += 1
...
test('T03 complete provider response with invalid model JSON is not retried', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-t03' })
  let calls = 0
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async (_url, init) => {
    calls += 1
    const model = JSON.parse(init.body).model
...
test('T04 upstream 401 is not retried', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-t04' })
  let calls = 0
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async () => {
    calls += 1
...
test('T05 valid semantic output rejected by role gate is not retried', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-t05', semanticRole: 'action_required' })
  let calls = 0
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async (_url, init) => {
    calls += 1
...
test('T06 and T07 ledger permits at most two attempts and retains the first failure', async () => {
  const ledger = new E2R4RunLedger({ storage: new MemoryStorage() })
  ...
  const target = { observationId: 'r4-ledger-target', phase: 'smoke', modelAlias: 'flash', inputSha256: HASH, phaseManifestSha256: HASH, maxAttempts: 2 }
  assert.equal((await call('/register', buildRegistration('r4-ledger', target))).status, 201)
...
test('T08 retry cannot create a second observation and duplicate reserve is rejected', async () => {
  const { ledger, env, body } = await primeSmoke({ runLabel: 'r4-t08' })
  let calls = 0
  const fetcher = async (_url, init) => {
    calls += 1
    const model = JSON.parse(init.body).model
  ...
  assert.equal((await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, fetcher)).status, 200)
  const callsAfterFirst = calls
  assert.equal((await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, fetcher)).status, 409)
...
test('generation firewall rejects evaluation keys before provider invocation', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-firewall' })
  let calls = 0
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', { ...body, expected: { tasks: [] } }), env, async () => { calls += 1; throw new Error('must not call') })
...
test('evaluation role and rubric fields never enter the provider payload', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-provider-firewall' })
  let providerBody
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async (_url, init) => {
...
test('server-authoritative lineage overrides model-authored modelName', async () => {
  const { env, body } = await primeSmoke({ runLabel: 'r4-lineage', modelAlias: 'pro' })
  const response = await runE2R4Benchmark(request('/api/experiments/e2-9/r4/benchmark/generate', body), env, async (_url, init) => successfulProviderResponse(JSON.parse(init.body).model, informationResult('deepseek-v4-flash')))
  assert.equal(response.status, 200)
  const payload = await response.json()
...
test('main Worker keeps R4 endpoint hidden when Preview flag is false', async () => {
  const worker = createWorker({ fetcher: async () => { throw new Error('upstream must not be called') } })
  const response = await worker.fetch(new Request('https://preview.example.test/api/experiments/e2-9/r4/benchmark/state?runLabel=r4-hidden', { headers: { origin: 'https://preview.example.test' } }), {
    E2_R4_BENCHMARK_ENABLED: 'false', E2_R4_PREVIEW_ORIGIN: 'https://preview.example.test',
