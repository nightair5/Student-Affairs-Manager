...
export function resolveWorkspaceTimeZone(timeZone) {
...
export function zonedLocalDateTimeToInstant(value, timeZone = DEFAULT_WORKSPACE_TIMEZONE) {
    ...
    return Object.keys(parts).every((key) => parts[key] === roundTrip[key])
        ? candidate
        : null;
}
export function parseBusinessDateTime(value, timeZone = DEFAULT_WORKSPACE_TIMEZONE) {
    if (isDateOnly(value))
        return null;
...
export function instantToWallClock(value, timeZone = DEFAULT_WORKSPACE_TIMEZONE) {
    const parts = partsInTimeZone(value, resolveWorkspaceTimeZone(timeZone));
    return new Date(Date.UTC(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second));
}
export function wallClockToInstant(value, timeZone = DEFAULT_WORKSPACE_TIMEZONE) {
...
function normalizeOcrTimeText(value) {
    return value
        .normalize('NFKC')
...
function calendarDateFromDateOnly(value) {
    if (!isDateOnly(value))
        return null;
...
function referenceCalendarDate(referenceTime, timezone) {
...
function inferredYear(month, reference) {
    if (reference.month >= 11 && month <= 2)
        return reference.year + 1;
    return reference.year;
}
function absoluteDateCandidates(text, reference) {
    const results = [];
    ...
    for (const match of text.matchAll(numericPattern)) {
        if (results.some((item) => (match.index ?? 0) >= item.index && (match.index ?? 0) < item.end))
            continue;
...
function relativeDateCandidates(text, reference) {
    const results = [];
    ...
    for (const match of text.matchAll(pattern)) {
        ...
        if (value === '明天')
            target = addCalendarDays(reference, 1);
        else if (value === '后天')
            target = addCalendarDays(reference, 2);
...
function convertClockHour(hour, period) {
...
function vagueTimeAst(rawText, type, timezone, issues) {
    return {
        rawText,
        type,
 ...
 */
export function parseChineseTimeAst(rawText, options) {
    const timezone = options.timezone ?? DEFAULT_WORKSPACE_TIMEZONE;
    if (!isValidTimeZone(timezone))
        return vagueTimeAst(rawText, options.type, timezone, ['invalid_timezone']);
    const reference = referenceCalendarDate(options.referenceTime, timezone);
    if (!reference)
        return vagueTimeAst(rawText, options.type, timezone, ['invalid_reference_time']);
    ...
    const baseDate = firstDate ?? (inheritedDate ? { ...inheritedDate, index: 0, end: 0, kind: 'absolute' } : null);
    const dateKind = firstDate?.kind ?? (inheritedDate ? 'inherited' : 'none');
    if (baseDate && !validCalendarDate(baseDate))
        return vagueTimeAst(rawText, options.type, timezone, ['invalid_calendar_date']);
    ...
    if (dates[1] && !isDateRange)
        return vagueTimeAst(rawText, options.type, timezone, ['conflicting_dates']);
    if (dates[1] && isDateRange && !validCalendarDate(dates[1]))
        return vagueTimeAst(rawText, options.type, timezone, ['invalid_range_end_date']);
    ...
    if (!firstClock.valid)
        return vagueTimeAst(rawText, options.type, timezone, ['invalid_clock']);
    if (firstClock.ambiguous)
        return vagueTimeAst(rawText, options.type, timezone, ['ambiguous_day_period']);
    ...
    let rangeEndNormalizedValue = null;
    ...
    if (secondClock && !isClockRange)
        return vagueTimeAst(rawText, options.type, timezone, ['conflicting_clocks']);
    if (secondClock && isClockRange) {
        if (!secondClock.valid || secondClock.ambiguous)
            return vagueTimeAst(rawText, options.type, timezone, ['invalid_or_ambiguous_range_end']);
        let endDate = dates[1] && dates[1].index <= secondClock.index ? dates[1] : baseDate;
        ...
        if (!dates[1] && (explicitNextDay || periodCrossesMidnight || secondsEnd <= secondsStart))
