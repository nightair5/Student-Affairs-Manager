...
function mapRecognition(workspace: WorkspaceData): { recognitionRuns: RecognitionRun[]; extractionDrafts: ExtractionDraft[] } {
  ...
  const extractionDrafts = workspace.drafts.map((draft): ExtractionDraft => ({
    ...
    commitOperationIds: [],
    acceptedEntityTempIds: draft.items.filter((item) => item.status === '已确认').map((item) => item.suggestion.id),
    rejectedEntityTempIds: draft.items.filter((item) => item.status === '已拒绝').map((item) => item.suggestion.id),
...
function mapTimePoints(workspace: WorkspaceData, timezone: string, now: string): TimePoint[] {
  const mapped = workspace.timePoints.map((item): TimePoint => ({
    ...
    eventId: null,
    relatedTaskIds: item.relatedTaskIds?.length ? item.relatedTaskIds : item.taskId ? [item.taskId] : [],
  ...
  workspace.tasks.forEach((task) => {
    ...
    mapped.push({
      ...
      relatedTaskIds: [task.id], relatedMaterialIds: [],
