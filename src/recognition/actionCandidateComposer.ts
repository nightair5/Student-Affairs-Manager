...
export const ACTION_CANDIDATE_COMPOSER_VERSION = 'action-candidate-composer-1.0.0' as const
...
export interface ActionCandidateComposition {
  composerVersion: typeof ACTION_CANDIDATE_COMPOSER_VERSION
...
function directive(candidate: LocalActionCandidate, objectCandidateId: string): ReducedDirectiveAnchor | null {
  const object = selectedObject(candidate, objectCandidateId)
  if (!object) return null
  return {
    anchorId: candidate.id,
    propositionScopeIds: [...candidate.propositionScopeIds],
...
export async function composeActionCandidates(
  ...
  const catalogIssues = await validateLocalActionCandidateCatalog(catalog, index)
  ...
  for (const candidate of catalog.candidates) {
    ...
    if (candidate.localDisposition === 'local_proposition') {
      ...
      const anchor = objectCandidateId ? directive(candidate, objectCandidateId) : null
      if (anchor) {
    ...
    const anchor = classification.objectCandidateId ? directive(candidate, classification.objectCandidateId) : null
    if (!anchor) {
      ledger.push({
        candidateId: candidate.id,
        anchorId: null,
  ...
  const semanticCoverageComplete = quarantined === 0 && catalog.unresolvedActionScopeIds.length === 0
  const value: ActionCandidateComposition = {
    composerVersion: ACTION_CANDIDATE_COMPOSER_VERSION,
...
export function deriveRequiresActionWithCoverage(
