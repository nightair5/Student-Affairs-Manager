...
export function validateRecognitionQuality(
  ...
  result.evidence.forEach((item) => {
    ...
    if (!quote || !sourceContent.includes(quote)) add({ code: 'EVIDENCE_NOT_SUPPORTED', severity: 'error', repairable: false, message: '证据不是来源正文中的连续逐字段落。', entityId: item.id, evidence: quote || null })
  })
  ...
  result.timePoints.forEach((item) => {
    ...
    if ((item.normalizedValue && SENTINEL_DATE.test(item.normalizedValue)) || SENTINEL_DATE.test(item.rawText)) {
      add({ code: 'FALSE_PRECISION', severity: 'error', repairable: true, message: '未知时间使用了哨兵日期。', entityId: item.tempId, evidence: item.normalizedValue })
    }
    if ((item.precision === 'relative' || item.precision === 'vague') && (item.normalizedValue !== null || !item.needsConfirmation)) {
      add({ code: 'FALSE_PRECISION', severity: 'error', repairable: true, message: '模糊或相对时间不得伪装为精确值。', entityId: item.tempId, evidence: item.rawText })
    }
    if ((item.precision === 'relative' || item.precision === 'vague') && !result.ambiguities.some((ambiguity) => ambiguity.evidenceIds.some((id) => item.evidenceIds.includes(id)))) {
      add({ code: 'MISSING_TIME_AMBIGUITY', severity: 'warning', repairable: true, message: '模糊时间缺少对应 Ambiguity。', entityId: item.tempId, evidence: item.rawText })
    }
  })
  ...
  sourceTimeTokens(sourceContent).forEach((token) => {
    const key = normalized(token)
    if (key && !representedTimes.some((actual) => actual.includes(key) || key.includes(actual))) {
