...
import {
  composeActionCandidates,
  deriveRequiresActionWithCoverage,
} from './actionCandidateComposer'
...
describe('candidate-level local composer and quarantine', () => {
  it('does not let the model delete local propositions or promote local non-tasks', async () => {
    ...
    const result = await composeActionCandidates(index, catalog, response, 'composer-run')
  ...
  it('quarantines only a broken needs-model candidate and preserves sibling output byte-for-byte', async () => {
    ...
    const baseline = await composeActionCandidates(index, catalog, response, 'composer-run')
    expect(baseline.ok).toBe(true)
    if (!baseline.ok) return
    const missing = structuredClone(response)
    missing.classifications.splice(0, 1)
    const partial = await composeActionCandidates(index, catalog, missing, 'composer-run')
  ...
  it('keeps unknown injected IDs out while all known siblings survive', async () => {
    ...
    expect(result.value.semanticCoverageComplete).toBe(true)
  ...
  it('reports unknown requiresAction when unresolved coverage remains', async () => {
    ...
    const partial = await composeActionCandidates(ambiguous.index, ambiguous.catalog, ambiguous.response, 'composer-run')
    expect(partial.ok).toBe(true)
    if (!partial.ok) return
    const partialFormation = formLocalTaskSuggestionsP4(ambiguous.index, partial.value.reduced)
    expect(deriveRequiresActionWithCoverage(partialFormation.requiresAction, partial.value)).toBeNull()
    ...
    const complete = await composeActionCandidates(nonTask.index, nonTask.catalog, nonTask.response, 'composer-run')
    expect(complete.ok).toBe(true)
    if (!complete.ok) return
    const completeFormation = formLocalTaskSuggestionsP4(nonTask.index, complete.value.reduced)
    expect(deriveRequiresActionWithCoverage(completeFormation.requiresAction, complete.value)).toBe(false)

    const unknownAction = await fixture('请领取纪念品。')
    const unknownComposition = await composeActionCandidates(unknownAction.index, unknownAction.catalog, unknownAction.response, 'composer-run')
    ...
    expect(deriveRequiresActionWithCoverage(false, unknownComposition.value)).toBeNull()
  })
  ...
  it('globally rejects a mismatched source binding or mutated local catalog', async () => {
    ...
    expect((await composeActionCandidates(index, catalog, rebound, 'composer-run')).ok).toBe(false)
    const tamperedCatalog = structuredClone(catalog)
    tamperedCatalog.candidates[0].action.surface = '核对'
    expect((await composeActionCandidates(index, tamperedCatalog, response, 'composer-run')).ok).toBe(false)
  })
})
