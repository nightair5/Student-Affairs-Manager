...
import {
  ...
  LOCAL_ACTION_CANDIDATE_V2_POLICY_VERSION,
  validateLocalActionCandidateCatalogV2,
  type LocalActionCandidateCatalog,
} from './localActionCandidateIndexV2'
import { indexLocalActionCandidates as indexLocalActionCandidatesV1 } from './localActionCandidateIndex'
...
describe('local immutable action candidate index v2', () => {
  it('is deterministic and binds every ID and UTF-16 span to the source', async () => {
    ...
    expect(first.catalog.candidates.every((item) => item.id.includes(LOCAL_ACTION_CANDIDATE_V2_POLICY_VERSION))).toBe(true)
  ...
  it('cryptographically separates the V2 policy from a V1 catalog, including empty input', async () => {
    for (const sourceText of ['请保存甲表。', '今天晴朗。']) {
      const index = await indexImmutableScopesV11('policy-bound-source', 'source-v1', sourceText)
      const v1 = await indexLocalActionCandidatesV1(index)
      const v2 = await indexLocalActionCandidatesV2(index)
      expect(v2.catalogFingerprint).not.toBe(v1.catalogFingerprint)
      expect((await validateLocalActionCandidateCatalogV2(v1 as unknown as LocalActionCandidateCatalog, index)).map((issue) => issue.code)).toContain('CATALOG_FINGERPRINT_MISMATCH')
    }
  })
  ...
  it('keeps an unclassified no-candidate scope unresolved instead of claiming no task', async () => {
    const { index, catalog: value } = await catalog('今天晴朗。')
    expect(value.candidates).toEqual([])
    expect(value.unresolvedActionScopeIds).toEqual([index.scopes[0].id])
    expect(await validateLocalActionCandidateCatalogV2(value, index)).toEqual([])
  })
  ...
  it('rejects any catalog mutation by recomputing the complete local derivation', async () => {
    ...
    expect((await validateLocalActionCandidateCatalogV2(tampered, index)).map((issue) => issue.code)).toEqual([
      'CATALOG_DERIVATION_MISMATCH',
    ])
    const rebound = await indexImmutableScopesV11('other-source', 'source-v1', '请保存核对记录。')
    expect((await validateLocalActionCandidateCatalogV2(value, rebound)).map((issue) => issue.code)).toContain('CATALOG_SOURCE_BINDING_MISMATCH')
  })
})
