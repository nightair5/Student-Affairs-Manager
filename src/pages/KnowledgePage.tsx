import { BookOpenCheck, CheckCircle2, Cloud, Download, FileSpreadsheet, FolderOpen, LoaderCircle, Search, ShieldCheck, Sparkles } from 'lucide-react'
...
import { buildObsidianVault, downloadKnowledgeCsv, downloadObsidianZip, writeObsidianFolder } from '../lib/obsidian'
  ...
  return <main className="page knowledge-page">
    ...
    {!localSearchAuthorizedAt ? <section className="knowledge-consent" aria-labelledby="knowledge-consent-title">
      ...
      <section className="knowledge-query" aria-labelledby="knowledge-query-title">
        ...
        <button className="example-fill" type="button" onClick={() => setQuestion('请根据任务、项目、来源、材料和历史，整理一份知识库导览，并指出最值得维护的三类资料。')}>填入“整理知识库”提问</button>
    ...
    <section className="obsidian-export" aria-labelledby="obsidian-export-title">
      ...
      <div className="obsidian-actions"><button className="secondary-button" type="button" disabled={!selectedTasks.length} onClick={() => { downloadObsidianZip(vault); setExportMessage(`已下载包含 ${vault.length} 个 Markdown 文件的 ZIP。`) }}><Download size={16} />下载 Markdown ZIP</button><button className="secondary-button" type="button" disabled={!selectedTasks.length} onClick={() => { downloadKnowledgeCsv(selectedTasks, sources.filter((source) => selectedSourceIds.has(source.id)), projects.filter((project) => selectedProjectIds.has(project.id))); setExportMessage('已下载知识库 CSV 索引，可用 Excel 或表格应用打开。') }}><FileSpreadsheet size={16} />下载 CSV 索引</button><button className="primary-button" type="button" disabled={!selectedTasks.length} onClick={() => void chooseFolder()}><FolderOpen size={16} />选择文件夹导出</button></div>
