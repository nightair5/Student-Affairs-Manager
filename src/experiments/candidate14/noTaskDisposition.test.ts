...
import {archiveCandidate14NoTaskInformation,candidate14FactsSha256,CANDIDATE14_ENGINEERING_DATABASE} from './noTaskDisposition'
...
describe('candidate14 no-task archive',()=>{
  it('atomically archives information and creates no task/calendar/reminder facts',async()=>{
    ...
    const result=await archiveCandidate14NoTaskInformation(transport,{workspaceRevision:workspaceSnapshotHash(workspace),sourceId:'source',draftId:'draft',operationId:'archive-1',firstOutputSha256:await candidate14FactsSha256(response),informationScopeIds:['notice'],eventTempIds:['e1'],timePointTempIds:['tp1'],archivedAt:'2026-09-22T00:01:00.000Z'})
