...
import {archiveCandidate14NoTaskInformation,candidate14FactsSha256,CANDIDATE14_ENGINEERING_DATABASE,persistCandidate14FirstOutputAnchor} from './noTaskDisposition'
...
describe('candidate14 no-task archive',()=>{
  it('atomically archives information and creates no task/calendar/reminder facts',async()=>{
    ...
    const anchored=await persistCandidate14FirstOutputAnchor(transport,'run','draft',now)
    const result=await archiveCandidate14NoTaskInformation(transport,{workspaceRevision:workspaceSnapshotHash(anchored.workspace),sourceId:'source',draftId:'draft',operationId:'archive-1',firstOutputSha256:await candidate14FactsSha256(response),informationScopeIds:['notice'],eventTempIds:['e1'],timePointTempIds:['tp1'],archivedAt:'2026-09-22T00:01:00.000Z'})
  ...
  it('rejects a result changed after the immutable first-output anchor and a replay with different receipt facts',async()=>{
    ...
    const memory=new MemoryWorkspaceRecordStore(),transport=Object.assign(memory,{name:CANDIDATE14_ENGINEERING_DATABASE}),repo=new CanonicalWorkspaceRepository(transport);await repo.save(workspace)
    const anchored=await persistCandidate14FirstOutputAnchor(transport,'run','draft',now),changed=structuredClone(anchored.workspace);changed.extractionDrafts[0].result!.sourceSummary.summary='锚点冻结后被改写';await repo.save(changed)
    await expect(archiveCandidate14NoTaskInformation(transport,{workspaceRevision:workspaceSnapshotHash(changed),sourceId:'source',draftId:'draft',operationId:'archive-3',firstOutputSha256:await candidate14FactsSha256(original),informationScopeIds:['notice'],eventTempIds:[],timePointTempIds:[],archivedAt:'2026-09-22T00:01:00.000Z'})).rejects.toThrow('CANDIDATE14_FIRST_OUTPUT_DRIFT')
  })
})
