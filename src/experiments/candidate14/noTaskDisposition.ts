...
export const CANDIDATE14_NO_TASK_DISPOSITION_VERSION='candidate14-no-task-disposition-1.2.0' as const
export const CANDIDATE14_ENGINEERING_DATABASE='rco-candidate14-d7-engineering-3' as const
...
export interface Candidate14FirstOutputAnchor {version:'candidate14-first-output-anchor-1';sourceVersionId:string;firstOutputSha256:string;frozenAt:string}
...
export async function buildCandidate14FirstOutputAnchor(result:NonNullable<WorkspaceV8['extractionDrafts'][number]['result']>,sourceVersionId:string,frozenAt:string):Promise<Candidate14FirstOutputAnchor>{
  if(!sourceVersionId||!Number.isFinite(Date.parse(frozenAt)))throw Error('CANDIDATE14_FIRST_OUTPUT_ANCHOR')
  return {version:'candidate14-first-output-anchor-1',sourceVersionId,firstOutputSha256:await candidate14FactsSha256(result),frozenAt}
}
...
export async function archiveCandidate14NoTaskInformation(transport:WorkspaceRecordStore&{name:string},request:Candidate14NoTaskArchiveRequest){
  ...
  const anchor=beforeDraft?.legacyData?.candidate14FirstOutputAnchor as unknown as Candidate14FirstOutputAnchor|undefined
  if(!beforeDraft?.result||!beforeRun||anchor?.version!=='candidate14-first-output-anchor-1'||anchor.sourceVersionId!==beforeRun.sourceVersionId||anchor.firstOutputSha256!==request.firstOutputSha256||await candidate14FactsSha256(beforeDraft.result)!==anchor.firstOutputSha256)throw Error('CANDIDATE14_FIRST_OUTPUT_DRIFT')
  ...
  const saved=await repository.transaction(current=>{
    ...
    const currentAnchor=draft.legacyData?.candidate14FirstOutputAnchor as unknown as Candidate14FirstOutputAnchor|undefined
    if(JSON.stringify(sorted(result))!==beforeResultCanonical||currentAnchor?.firstOutputSha256!==request.firstOutputSha256||currentAnchor.sourceVersionId!==version.id)throw Error('CANDIDATE14_FIRST_OUTPUT_DRIFT')
