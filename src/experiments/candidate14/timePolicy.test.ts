...
describe('candidate14 time policy',()=>{
  it('keeps tentative afternoon imprecise and reviewable',()=>{
    ...
    expect(value.precision).toBe('vague');expect(value.needsConfirmation).toBe(true);expect(value.uncertainty).toBe('tentative')
  })
  it('keeps unknown notification time unresolved',()=>{
    ...
    expect(value.normalizedValue).toBeNull();expect(value.uncertainty).toBe('unknown')
  })
})
