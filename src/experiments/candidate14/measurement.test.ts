...
import {aggregateCandidate14Trials,calculateCandidate14Trial,CANDIDATE14_MEASUREMENT_DATABASE,registerCandidate14Trial,type Candidate14MetricEvent,type Candidate14TrialRegistration} from './measurement'

const sha='a'.repeat(64),snapshot='b'.repeat(64)
const registration:Candidate14TrialRegistration={registrationId:'reg-1',trialId:'t1',sourceIdentitySha256:sha,candidateIdentitySha256:'c'.repeat(64),origin:'HUMAN_TRIAL',status:'REGISTERED',registeredAtMs:0}
...
describe('candidate14 product metrics',()=>{
  it('requires a persisted registration and ordered commit/readback',async()=>{
    const store=makeStore();await registerCandidate14Trial(store,registration)
    ...
    expect(good.correctDisposition).toBe(true);expect(good.registryVerified).toBe(true);expect(aggregateCandidate14Trials([good],[registration]).humanTrials).toBe(1)
