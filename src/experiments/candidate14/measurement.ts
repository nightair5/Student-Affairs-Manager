...
export interface Candidate14TrialRegistration {registrationId:string;trialId:string;sourceIdentitySha256:string;candidateIdentitySha256:string;firstOutputSha256:string;origin:Candidate14Origin;status:'REGISTERED';registeredAtMs:number}
...
export async function calculateCandidate14Trial(store:Candidate14MeasurementStore,registration:Candidate14TrialRegistration,events:readonly Candidate14MetricEvent[],judgment:Candidate14Judgment):Promise<Candidate14TrialMetrics>{
  ...
  if(starts.length!==1||snapshots.length!==1||snapshots[0].snapshotSha256!==registration.firstOutputSha256||!end||startIndex!==0||!(startIndex<snapshotIndex&&snapshotIndex<interactiveIndex)||endIndex!==rows.length-1||starts[0].atMs<registration.registeredAtMs)return empty('INCOMPLETE',registration,true)
  ...
  const operations=new Set(rows.flatMap((event,index)=>event.kind==='edit_activity'&&event.editCategory&&CANDIDATE14_EDIT_CATEGORIES.includes(event.editCategory as Candidate14EditCategory)&&event.operationId&&rows.some((commit,commitIndex)=>commit.kind==='commit_succeeded'&&commit.operationId===event.operationId&&commitIndex>index&&rows.some((readback,readbackIndex)=>readback.kind==='readback_verified'&&readback.operationId===event.operationId&&readbackIndex>commitIndex))?[event.operationId]:[]))
