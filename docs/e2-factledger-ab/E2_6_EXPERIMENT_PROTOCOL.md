...
Path A is one direct `recognition-2.4.1` call. Path B is `fact-ledger-extraction-1.0.0` → deterministic FactLedger validation → `fact-ledger-planner-1.0.0`. Router, production Validator and Repair do not participate. Both paths use `deepseek-v4-flash`, temperature 0, disabled thinking, `max_tokens=8192`, JSON mode and zero harness retries.
