...
- 部署版本：`6e1f08e2-5c63-4d01-94d6-852ccb5810ff`。两个网址共 16 项资源比对一致，记录于 ONLINE.json。
- 只允许两个固定 HTTPS origin；不接受任意域名，也不允许生产首页进入实验配置。新增加载中、加载失败与 JavaScript 未启用提示，避免资源未到达时完全空白。经原有保密审查后压缩脚本，主脚本由 2,708,468 字节降为约 1,600,300 字节。
...
最终同步：修复提交 `479ccba940080673dfcf93acdb2d2e57017bcee8` 已快进合入并推送原实验分支；在原工作目录重新执行专用 2/2 测试通过，并从该目录重建部署，避免隔离目录依赖链接产生的打包差异。最终两域名 16 项资源再次逐字一致。隔离目录的归档换行恢复重跑曾触发历史字节校验失败，未修改历史校验值；原实验目录最终专用构建通过。原实验目录已有的全部未提交实验修改继续保留。
