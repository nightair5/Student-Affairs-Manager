...
首次输出锚点在识别运行成功后写入独立的追加式历史记录，归档流程只读该记录，不从可编辑草稿元数据重建。真人试次必须由独立协调方预置与来源、候选和首次输出哈希一致的授权登记；未预置时注册失败，四项真人指标保持 `NOT_OBSERVABLE`。编辑只有满足“编辑 → 同操作提交 → 同操作独立读回”的顺序才计入主动修改。
