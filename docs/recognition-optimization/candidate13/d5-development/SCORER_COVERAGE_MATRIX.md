...
`candidate13-scoring-4.0.0`是新的 Development 评估器。D4 Candidate13 的 Prompt、请求构造和原v3绑定均不改；D5 Manifest 会另行记录新评估绑定。
...
| 时间原文、归一值、类型、精度、actionable | `timePoints` | 按任务归属的结构化严格集合相等 | Major | 精确/模糊/未给出 |
...
参照合同尚未表达材料数量和提交渠道的可接受变体，因此v4不对它们做伪精确评分；该限制进入能力缺口，不通过改生产Schema规避。
