...
- R11独立登记任务与Q07新PDF任务各1项内存确认/重开读回通过。
- R11送样前置未确认仍拒绝；R12旧实体缺失/坏引用保持拒绝。
- 模拟核对不是用户真实材料状态，不是浏览器PASS，不是模型准确率提高。
...
- 原7未提交业务接线保留并纳入工程交付，不冒充已通过真实页面验收。
...
- 独立new repository为MemoryStore工程证据，不是实际IndexedDB。
...
- 同Worker：student-affairs-real-input-preview。
- 当前版本0343dcfe-d36a-4173-aa09-37960f40dcef。
- 只上传变化的browser.js，7线上资产SHA匹配，模型POST405。
- /api/status明确modelCallsEnabled:false；只绑定ASSETS，没有Secret/后端库。
- Q01/Q07及R11/R12匿名历史回放保留，不代表采用candidate07。
- 当前域名库rco-mainline-01-02-i1-real-input-https-preview-1。
- 不读6631，不清库，不创建替代库；当前真实HTTPS任务数未观测。
- 已有库恢复，坏库不自动建空库或fallback。

## 浏览器与下一步
- 本轮无明确官方控制恢复消息，因此0连接尝试，不重装/重启/诊断。
- HTTPS真实核对、确认、刷新找回、IndexedDB、实际下载均NOT_VERIFIED。
- R12仍作负例，已有Q07有效替代作正常成功对照，失败原答不改。
- 控制恢复后助手操作，否则仅一次最少人工/文件交接；不反复要求刷新。
- 需操作前独立读库，明确材料unknown/确认正确任务，刷新后读库及实际JSON下载。
- 下载与独立仓储必须两个实际读取路径，不能同文件双哈希。
- 下一步唯一优先补真实HTTPS证据，不新增模型、候选、数据或付费批次。
- 本轮工程与部署完成不等于整体产品真实验收完成；完成当前交付后停止。
