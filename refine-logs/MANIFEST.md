...
阶段：C12-D3未来评分契约修复；日期：2026-09-21。作者：本任务；性质：评分参照、compiler、validator、scorer、未来预注册与零调用验证。
A1—A5、B1、B2、Candidate12冻结、D1身份准备、D2执行和D3契约修复已完成。D3新增模型调用、Secret读取和账本写入均为0；没有真人标签、正式Holdout、合并或部署。
...
| [当前计划](EXPERIMENT_PLAN.md) | c12-plan-0.9-d3-scorer-contract | D3后的权威当前计划；时间戳文件保留快照 |
| [时间戳跟踪表](2026-09-21_1100_EXPERIMENT_TRACKER.md) | c11-tracker-0.1 | 本轮任务状态快照 |
| [当前跟踪表](EXPERIMENT_TRACKER.md) | c12-tracker-0.9-d3-scorer-contract | D3真实状态；时间戳文件保留原快照 |
...
| [短交接](../docs/recognition-optimization/CURRENT_CONTEXT.md) | C12-D3 | 下一轮恢复入口；先冻结Candidate13或准备全新数据，正式Holdout仍等待真实独立人工标签 |
