...
版本：c11-tracker-0.1。日期：2026-09-21。
...
|---|---|---|---|---:|---|---|
| C11-000 | P0 | 隔离worktree、分支、交接与规划 | 历史只读 | 0 | LOCAL_VALIDATED | 文档检查、提交和远端核验见分支审计 |
| C11-010 | P1 | scorer v2及一对一/字段反例 | 工程合成 | 0 | PLANNED_NOT_AUTHORIZED | 新增反例>=30，正确/错误fixture明确分离 |
| C11-011 | P1 | 24旧答复算、不可变证据对照 | SEEN_DIAGNOSTIC | 0 | PLANNED_NOT_AUTHORIZED | 无Secret/账本写入，重评分另存 |
| C11-012 | P1 | 只读重放、元数据和临时预算失败分支 | 工程合成 | 0 | PLANNED_NOT_AUTHORIZED | repeated verify规范化结果相同，真实账本hash不变 |
| C11-020 | P2 | candidate11最小完成标准修正 | 工程合成 | 0 | PLANNED_NOT_AUTHORIZED | P1可信；版本化新模块和示例 |
| C11-021 | P2 | 正确事实→真实组件→确认仓储读回 | 独立工程库 | 0 | PLANNED_NOT_AUTHORIZED | 保留取消/未知/无日期，确认前0正式任务 |
...
| C11-050 | P5 | 新回答接真实App、本机事件和恢复 | 结算输出/独立库 | 0新增 | PLANNED_NOT_AUTHORIZED | P4净收益；适用浏览器/事务验收 |
...
本轮没有生成新数据集、正式freeze、请求或结果；旧账本644行/314次的快照见BRANCH_BASELINE.json。
...
历史RCO-5-007冻结hash失败保留在检查记录，文档阶段不重跑整个旧模型实验。
